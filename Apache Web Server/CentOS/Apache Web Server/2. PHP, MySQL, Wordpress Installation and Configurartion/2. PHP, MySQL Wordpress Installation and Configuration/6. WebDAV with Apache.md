
# **WebDAV-with-Apache**

**WebDAV with Apache**
WebDAV (Web Distributed Authoring and Versioning) extends the HTTP protocol to enable remote file management on a web server. Apache supports WebDAV using the `mod_dav` and `mod_dav_fs` modules.

### **Features of WebDAV**

* Access and manage files over HTTP/S.
* Supports creation, editing, moving, and deletion of files.
* Integrates with version control tools like Git and SVN.
* Clients can mount WebDAV directories as local drives.
* Suitable for collaborative environments and remote file hosting.

---

### **Install and Verify Apache**

**Check if Apache is Installed** (Verifies whether Apache is already installed)

```bash
rpm -qa | grep httpd

```

**Installs Apache and related packages on RHEL-based systems**

```bash
yum install httpd*

```

**Lists all currently active modules**

```bash
httpd -M

```

**Check for WebDAV Module Support** (Confirms that WebDAV support is enabled via `mod_dav_fs`)

```bash
httpd -M | grep fs

```

**Expected output:**

```bash
dav_fs_module (shared)

```

---

### **Create and Set Up WebDAV Directory**

**Create Directory** (Sets up the directory for WebDAV content)

```bash
mkdir /var/www/html/webdav

```

**Set Ownership** (Ensures Apache owns the directory)

```bash
chown apache:apache /var/www/html/webdav/

```

**Set Permissions**

```bash
chmod 777 /var/www/html/webdav/

```

*(For production: prefer 755 or 775 for better security.)*


---
### Firewall http Service add

```bash
firewall-cmd --permanent --add-service=http
```
```bash
firewall-cmd --reload
```
---

### **Configure Apache for WebDAV**

**Edit Apache Configuration**

```bash
vim /etc/httpd/conf/httpd.conf

```

**Add or update the Virtual Host block:**
(This enables WebDAV under the `/webdav` path)

```bash
<VirtualHost *:80>
    DocumentRoot /var/www/html/
    DirectoryIndex index.html
    <Directory /var/www/html/webdav>
        DAV On
    </Directory>
</VirtualHost>

```

---

### **Test WebDAV Directory Access**

**Use curl to Send OPTIONS Request** (Verifies supported HTTP methods by the server)

```bash
curl -v -X OPTIONS http://192.168.31.74/webdav/

```

---

### **Configure SELinux and Firewall**

**Restart Apache**

```bash
systemctl restart httpd.service

```

**Check SELinux Status**

```bash
sestatus

```

**Disable SELinux Temporarily** (Not Recommended for Production)

```bash
vim /etc/sysconfig/selinux

```

Change the value:

```bash
SELINUX=disabled

```

**Instead, prefer using proper context labeling:**

```bash
chcon -R -t httpd_sys_content_t /var/www/html/webdav

```

---

### **Uploading and Deleting Files Using curl**

**Upload a Text File**

```bash
curl -X PUT -d "hi" http://192.168.31.74/webdav/1.txt

```

**Upload a PHP File**

```bash
curl -X PUT -d "<?php phpinfo(); ?>" http://192.168.31.74/webdav/phpinfo.php

```

**Delete Files**

```bash
curl -X DELETE http://192.168.31.74/webdav/phpinfo.php

```

```bash
curl -v -X DELETE http://192.168.31.74/webdav/1.txt

```

---

### **Secure WebDAV with Basic Authentication**

**Create User Password File**

```bash
htpasswd -c /etc/httpd/htpasswd dev

```

**Add more users:**

```bash
htpasswd /etc/httpd/htpasswd admin

```

```bash
htpasswd /etc/httpd/htpasswd root

```

```bash
htpasswd /etc/httpd/htpasswd user

```

**View .htpasswd File**

```bash
cat /etc/httpd/htpasswd

```

**Sample output:**

```bash
dev:$apr1$GXmFNd4f$HBa.Fu2SB1SMUncBcATuG/
admin:$apr1$NmNcn0pS$Lhk2I/HVjXV7mKKpXis4C/
root:$apr1$Qey..Fqn$y2GIMsuP1JDDMrnWHgDnU.
user:$apr1$zp/J6/mB$v.h.deWHBq/34CABimwk3/

```

---

### **Configure Apache for WebDAV Authentication**

**Edit Apache Config Again**

```bash
vim /etc/httpd/conf/httpd.conf

```

**Update VirtualHost block:**

```bash
<VirtualHost *:80>
    DocumentRoot /var/www/html/
    DirectoryIndex index.html
    <Directory /var/www/html/webdav>
        DAV On
        AuthType Basic
        AuthName "webdav"
        AuthUserFile /etc/httpd/htpasswd
        Require valid-user
    </Directory>
</VirtualHost>

```

**Restart Apache After Config Change**

```bash
systemctl restart httpd.service

```

---

### **Notes for Production Environments**

* Always use HTTPS to secure credentials in transit.
* Use SELinux context labeling instead of disabling it.
* Apply firewall rules or IP whitelisting for added security.

```bash
chcon -R -t httpd_sys_content_t /var/www/html/webdav

```

---

### **WebDAV Client Testing**

**Test HTTP Methods**

```bash
curl -v -X OPTIONS http://192.168.31.74/webdav/

```

```bash
curl -i -k -X OPTIONS https://192.168.31.74/webdav/

```

**Scan WebDAV with Nmap**

```bash
nmap -v -sT -sV -A -O -p 80 --script=http-methods.nse --script-args http-methods.url-path='/webdav/' 192.168.31.74

```

---

### **Authenticated File Operations**

**OPTIONS with Auth**

```bash
curl -v -u dev:123 -X OPTIONS http://192.168.31.74/webdav/

```

```bash
curl -i -k -u dev:123 -X OPTIONS https://192.168.31.74/webdav/

```

**Authenticated File Upload**

```bash
curl -u dev:123 -X PUT -d "hi" http://192.168.31.74/webdav/1.txt

```

```bash
curl -X PUT -u dev:123 -d "<?php phpinfo(); ?>" http://192.168.31.74/webdav/phpinfo.php

```

**Authenticated File Deletion**

```bash
curl -u dev:123 -X DELETE http://192.168.31.74/webdav/phpinfo.php

```

```bash
curl -v -u root:123 -X DELETE http://192.168.31.74/webdav/1.txt

```

---

### **Using cadaver - WebDAV Command-Line Client**

**Install cadaver**

```bash
apt install cadaver

```

*or*

```bash
yum install cadaver

```

**Connect to WebDAV Server**

```bash
cadaver http://192.168.31.74/webdav/

```

**Sample cadaver Session**

```bash
dav:/webdav/> ?

```

```bash
dav:/webdav/> ls

```

```bash
dav:/webdav/> put network.png

```

```bash
dav:/webdav/> mkdir newdir

```

```bash
dav:/webdav/> mput *.php

```

```bash
dav:/webdav/> delete *

```

**Connect to a Different Path**

```bash
cadaver http://192.168.31.74/test/

```
